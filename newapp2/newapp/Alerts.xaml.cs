...
namespace newapp
{
    ...
    public sealed partial class Alerts : Page
    {
        ...
        private async void gridView_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            // await dialog.ShowAsync();
            List<Person> Host_Person = await App.MobileService.GetTable<Person>().Where(x => x.name == ev.hostname & x.contactno == ev.contactno).ToListAsync();
            if (Host_Person.Count != 1)
            {
                var dialog = new MessageDialog("Error");
                await dialog.ShowAsync();
            }
            this.Frame.Navigate(typeof(Profile2), Host_Person[0]);
        }
